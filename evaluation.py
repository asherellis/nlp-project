...
questions_file = f"NLP Project Data/{split}_final_jsonl.txt"

simple_output = f"{split}_simple_output.json"
strong_output = f"{split}_strong_output.json"
...
if strong_exists:
    ...
    llm_results = evaluate_llm_judge(questions, predictions, "corpus", lambda_weight=0.5)
    if llm_results:
...
output_file = f"{split}_evaluation.json"
