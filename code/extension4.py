...
Combines BM25 lexical matching with dense semantic embeddings to improve
retrieval performance. This handles vocabulary mismatch issues where
semantically similar content uses different terminology.

Pipeline:
1. BM25 retrieves top-N candidates (lexical matching)
2. Sentence-transformer computes semantic similarity scores
3. Scores are combined: hybrid_score = alpha * bm25_norm + (1-alpha) * semantic
4. Cross-encoder reranks top candidates
5. LLM generates answer from top document
6. LLM extracts citations
"""
import subprocess
import sys
